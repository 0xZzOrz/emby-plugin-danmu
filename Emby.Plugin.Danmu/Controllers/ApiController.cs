...
using System.Linq;
using Emby.Plugin.Danmu.Scrapers.Dandan.Entity;
using Emby.Plugin.Danmu.Controllers.Entity;
using Emby.Plugin.Danmu.Core;
using Microsoft.Extensions.Caching.Memory;

namespace Emby.Plugin.Danmu.Controllers
{
    [ApiController]
    [AllowAnonymous]
    public class ApiController : ControllerBase
    {
        ...
        private async Task<List<Emby.Plugin.Danmu.Scrapers.Dandan.Entity.Episode>> GetAnimeEpisodes(AbstractScraper scraper, string id)
        {
            var list = new List<Emby.Plugin.Danmu.Scrapers.Dandan.Entity.Episode>();
            try
            {
                var result = await scraper.GetEpisodesForApi(id).ConfigureAwait(false);
                foreach (var (ep, idx) in result.WithIndex())
                {
                    var episode = new Emby.Plugin.Danmu.Scrapers.Dandan.Entity.Episode()
                    {
        ...
        public async Task<ActionResult> DownloadByCommentID(string cid, int chConvert=0, bool withRelated=true, string format="json")
        {
            ...
            if (danmaku != null)
            {
                ...
                else
                {
                    ...
                    var result = new Emby.Plugin.Danmu.Scrapers.Dandan.Entity.CommentResult
                    {
                        Comments = danmaku.Items.Select(item => new Emby.Plugin.Danmu.Scrapers.Dandan.Entity.Comment
                        {
                            Cid = item.Id,
